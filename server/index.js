...
const corsOptions = {
  ...
  origin: [
    ...
    "https://app.vitalx.in",
    "https://dev.vitalx.in",
    "https://admirable-taiyaki-b2b323.netlify.app",
  ],
...
io.on("connection", (socket) => {
  socket.on(
    "requestPeopleAheadUpdate",
    async ({ doctorId, appointmentNumber }) => {
      try {
        const peopleAhead = await getPeopleAheadCount(
          appointmentNumber,
          doctorId
        );
...
app.post("/register", async (req, res) => {
  try {
    ...
    const { appointment, peopleAhead } = await insertAppointment(
      patient.patient_id,
      doctor_id
    );
...
app.post("/verifyOTP", async (req, res) => {
  ...
  try {
    ...
    if (isVerified) {
      res.json({
        success: true,
        patientExists: await findPatientByContactNumber(phoneNumber),
